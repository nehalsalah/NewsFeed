...
public final class QueryUtils {
    ...
    private static List<NewsFeed> extractFeatureFromJson(String earthquakeJSON) {
        ...
        try {
            ...
            JSONObject earthquake = baseJsonResponse.getJSONObject("response");
            // Extract the JSONArray associated with the key called "results",
            JSONArray NewsFeedArray = earthquake.getJSONArray("results");

            // For each result in the NewsFeedArray, create an {@link NewsFeed} object
            for (int i = 0; i < NewsFeedArray.length(); i++) {
                ...
                String Type = currentEarthquake.getString("type");
                // Extract the value for the key called "url"
                String url = currentEarthquake.getString("webUrl");
                // Extract the value for the key called "type"
                String News = currentEarthquake.getString("sectionName");
                //String News = NewsType[i];
                Log.d("search", "News: " + News);
                // Create a new {@link NewsFeed} object with the News,Type,webTitle,
                // and url from the JSON response.
                NewsFeed newsFeed = new NewsFeed(News, Type, webTitle, url);
